...
# APIRouter
# ---------------------------------------------------------------------------
simulateur_router_v3 = APIRouter(prefix="/simulateurv3", tags=["simulateurv3"])

# ---------------------------------------------------------------------------
# SOAP / Cert configuration (paths & constants alignés sur le fichier d'origine)
# ---------------------------------------------------------------------------
SOAP_WSDL_URL = "https://10.101.1.160:8443/xmlvend/xmlvend.wsdl"
SOAP_SERVICE_NAME = "TrialCreditVendRequest"  # conservé pour header SOAPAction
...
# ---------------------------------------------------------------------------
# Modèles Pydantic (identiques à l'original pour compatibilité)
# ---------------------------------------------------------------------------
...
def _extract_endpoint_from_wsdl(wsdl_xml: str) -> Optional[str]:
    """
    Essaie d'extraire l'URL d'endpoint depuis le WSDL (soap:address/@location).
...
def parse_soap_response(xml_content: str, request_date_time: str, unique_number: str, http_status: int) -> dict:
    """
    Parsing identique au fichier initial: retourne un JSON structuré.
    """
    try:
...
# ---------------------------------------------------------------------------
@simulateur_router_v3.post("/trial-credit-vend-request")
async def trial_credit_vend_request(request: TrialCreditVendRequest):
    ...
    try:
        ...
        # 1) Valider l'accès WSDL avec le PFX (comme SoapUI)
        ...
        wsdl_resp = requests_pkcs12.get(
            ...
            timeout=30,
            verify=False  # même logique que ton code initial (session.verify=False)
        )
        wsdl_resp.raise_for_status()

        # 2) Déterminer l'endpoint depuis le WSDL ou fallback sur l'URL sans .wsdl
        ...
        # 4) POST SOAP avec PFX direct
        response = requests_pkcs12.post(
            endpoint_url,
            data=soap_xml,
            headers=headers,
            timeout=60,
            verify=False,  # idem: désactivé comme dans ton implémentation initiale
            pkcs12_filename=str(CERTIFICATE_PATH),
            pkcs12_password=CERTIFICATE_PASSWORD,
        )
...
@simulateur_router_v3.get("/health")
